using System;
using System.Web;
using LogEm.Site.HTTPHandlers.Data;

namespace LogEm.Site
{
    internal class DataHandlerFactory : IHttpHandlerFactory
    {
        /// <summary>
        /// Returns a data handler that implements the <see cref="IHttpHandler"/> 
        /// interface and which is responsible for serving the request.
        /// </summary>
        /// <returns>
        /// A new <see cref="IHttpHandler"/> data handler that processes the request.
        /// </returns>
        ...
        private static IHttpHandler InstantiateHandler(HttpContext pContext, string pRequestType, string pUrl, string pPathTranslated)
        {
            ...
            return new RequestsByBrowserHandler();
        }
